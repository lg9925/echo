...
- Tasks: `authoring` (the `/v1/compose` "想说" path), `gloss`, `scenario`.
- **Current test-phase default: all three → `anthropic` / `claude-sonnet-4-6`.** The constitution's intent is to send high-frequency/batch work to a cheaper model (DeepSeek); we get there **without code changes** via per-task env overrides:
