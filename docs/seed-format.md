...
### 两种岛形态:词句集 vs 问答对话(QA)

一个岛可以是两种形态之一,字段结构**完全相同**,差别只在内容编排:

- **词句集**:一组各自独立、都从学习者视角出发的实用句(如 `打招呼`)。彼此不必连成一段对话。
- **问答对话(QA)**:按真实流程**一来一回**的对话(如点餐、值机、看医生),既有**对方**(店员/服务员/工作人员)会问/会说的话,也有**你**的回答与提问,交替推进。

> 对话型岛**不引入结构化的"角色"字段**(保持数据简单)。谁在说话,通过 `note` **开头的说话方标记**来表达:`"对方:…"` 或 `"你:…"`(见 §4)。播放器照常一句一句跟读,只是内容读起来是连贯的问答。
>
> 像"打招呼"这种本就不是一段连续对话的主题,**不必强行编成对话**——保持词句集即可。
...
| `note` | string | ✅ | 用法说明(中文):何时用、正式/随意、地区差异、易错点等。一两句即可。**对话型岛**:在开头加说话方标记 `"对方:"` 或 `"你:"`,再跟说明,如 `"对方:服务员问要喝点什么。"`。词句集无需此前缀。 |
